...
def getwork(bitcoind, net, use_getblocktemplate=False, txidcache={}, feecache={}, feefifo=[], known_txs={}):
    ...
    ecash = get_ecash_work(work, net) if getattr(net.PARENT, 'ECASH_RULES', False) else {}
    if ecash:
        winners = yield get_ecash_staking_winners(bitcoind, work['previousblockhash'])
        if winners is not None:
            staking_winners[int(work['previousblockhash'], 16)] = winners
            while len(staking_winners) > 50:
                del staking_winners[min(staking_winners)]
...
# previous block hash (int) -> frozenset of payout scripts our node accepts as staking
# reward winners for the block built on it. Read by data.ECashShare.check().
staking_winners = {}

def get_ecash_work(work, net):
    '''Parse the eCash-specific getblocktemplate fields into the coinbase outputs
    that must be paid before the PPLNS split (see Bitcoin ABC src/rpc/mining.cpp).'''
    coinbasetxn = work.get('coinbasetxn', {})
    if 'minerfund' not in coinbasetxn:
        raise ValueError('getblocktemplate has no coinbasetxn.minerfund. Is this a Bitcoin ABC (eCash) node with -simplegbt disabled?')
    reserved_outputs = []
    minerfund = coinbasetxn['minerfund']
    if minerfund['addresses'] and int(minerfund['minimumvalue']) > 0:
        scripts = [bitcoin_data.address_to_script2(addr, net.PARENT) for addr in minerfund['addresses']]
        known = [x for x in scripts if x in net.PARENT.MINER_FUND_SCRIPTS]
        if not known:
            raise ValueError('Node reports unknown miner fund addresses %r; update MINER_FUND_SCRIPTS in p2pool/bitcoin/networks/%s.py' % (minerfund['addresses'], net.PARENT.NAME))
        reserved_outputs.append(dict(value=int(minerfund['minimumvalue']), script=known[0]))
    if 'stakingrewards' in coinbasetxn:
        stakingrewards = coinbasetxn['stakingrewards']
        reserved_outputs.append(dict(value=int(stakingrewards['minimumvalue']), script=stakingrewards['payoutscript']['hex'].decode('hex')))
    return dict(
        reserved_outputs=reserved_outputs,
        payout_subsidy=work['coinbasevalue'] - sum(o['value'] for o in reserved_outputs), # what PPLNS actually splits
        rtt_target=bitcoin_data.FloatingIntegerType().unpack(work['rtt']['nexttarget'].decode('hex')[::-1]).target if 'rtt' in work else None,
    )

@defer.inlineCallbacks
def get_ecash_staking_winners(bitcoind, previousblockhash):
    '''All payout scripts the node accepts as staking reward winners, or None if unknown.'''
    try:
        winners = yield bitcoind.rpc_getstakingreward(previousblockhash)
    except jsonrpc.Error: # not ready yet, avalanche disabled, or method missing
        defer.returnValue(None)
    defer.returnValue(frozenset(w['hex'].decode('hex') for w in winners) or None)
