...
import p2pool
from p2pool.bitcoin import data as bitcoin_data, helper as bitcoin_helper, script, sha256
...
def load_share(share, net, peer_addr):
    ...
    if (share['type'] == ECashShare.VERSION) != (getattr(net, 'GENESIS_SHARE_VERSION', None) == ECashShare.VERSION):
        from p2pool import p2p
        raise p2p.PeerMisbehavingError('share type %i not allowed on this network' % (share['type'],))
...
def get_genesis_share_type(net):
    return share_versions[getattr(net, 'GENESIS_SHARE_VERSION', Share.VERSION)]
...
class BaseShare(object):
    ...
    def get_dynamic_types(cls, net):
        ...
        t['share_info_type'] = pack.ComposedType([
            ('share_data', pack.ComposedType([
                ...
                ] + ([('reserved_outputs', pack.ListType(pack.ComposedType([ # paid before the PPLNS split, e.g. eCash miner fund and staking reward
                    ('value', pack.IntType(64)),
                    ('script', pack.VarStrType()),
                ])))] if cls.VERSION >= 36 else []) + [
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None, segwit_data=None):
        ...
        reserved_outputs = share_data.get('reserved_outputs', [])
        if len(reserved_outputs) > 4 or sum(o['value'] for o in reserved_outputs) > share_data['subsidy']:
            raise ValueError('invalid reserved outputs')
        payout_subsidy = share_data['subsidy'] - sum(o['value'] for o in reserved_outputs)

        amounts = dict((script, payout_subsidy*(199*weight)//(200*total_weight)) for script, weight in weights.iteritems()) # 99.5% goes according to weights prior to this share
        ...
        if sum(amounts.itervalues()) != payout_subsidy or any(x < 0 for x in amounts.itervalues()):
            raise ValueError()
        ...
        gentx = dict(
            ...
            tx_outs=([dict(value=0, script='\x6a\x24\xaa\x21\xa9\xed' \
                    ...
                    + [dict(value=o['value'], script=o['script']) for o in reserved_outputs] \
                    + payouts \
...
class ECashShare(BaseShare):
    VERSION = 36
    VOTING_VERSION = 36
    SUCCESSOR = None
    MINIMUM_PROTOCOL_VERSION = 3600

    def check_ecash_outputs(self):
        parent = self.net.PARENT
        subsidy = self.share_data['subsidy']
        outputs = self.share_data['reserved_outputs']
        # The miner fund is a consensus rule and depends only on share data, so every node agrees: reject.
        if not any(o['script'] in parent.MINER_FUND_SCRIPTS and o['value'] >= parent.MINER_FUND_RATIO*subsidy//100 for o in outputs):
            raise ValueError('share does not pay the eCash miner fund')
        # The staking reward winner comes from each node's avalanche view, so we can only
        # judge shares built on the block our node is working on, and only punish, not reject.
        winners = bitcoin_helper.staking_winners.get(self.header['previous_block'])
        if winners and not any(o['script'] in winners and o['value'] >= parent.STAKING_REWARD_RATIO*subsidy//100 for o in outputs):
            print 'Share %064x does not pay an eCash staking reward winner known to our node. Naughty.' % (self.hash,)
            self.naughty = 1
...
share_versions = {s.VERSION:s for s in [ECashShare, PaddingBugfixShare, SegwitMiningShare, NewShare, PreSegwitShare, Share]}
