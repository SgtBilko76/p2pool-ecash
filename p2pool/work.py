...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_address, donation_percentage, merged_urls,
        ...
        def compute_work():
            ...
            # eCash: skipping ahead would build a template without the miner fund and staking reward outputs of the next block
            if bb is not None and not getattr(self.node.net.PARENT, 'ECASH_RULES', False) and bb['previous_block'] == t['previous_block'] and self.node.net.PARENT.POW_FUNC(bitcoin_data.block_header_type.pack(bb)) <= t['bits'].target:
        ...
        if random.uniform(0, 100) < self.worker_fee:
            ...
            try:
                if self.node.best_share_var.value is not None:
                    ...
                    share_type = p2pool_data.get_genesis_share_type(self.node.net)
    ...
    def get_work(self, user, address, desired_share_target,
        ...
        if desired_share_target is None:
            ...
            block_subsidy = self.node.bitcoind_work.value.get('payout_subsidy', self.node.bitcoind_work.value['subsidy'])
        ...
        if True:
            share_info, gentx, other_transaction_hashes, get_share = share_type.generate_transaction(
                ...
                share_data=dict(
                    ...
                    **(dict(reserved_outputs=self.current_work.value['reserved_outputs']) if share_type.VERSION >= 36 else {})
                ),
            ...
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    if pow_hash <= header['bits'].target:
                        ...
                        print
                        rtt_target = self.current_work.value.get('rtt_target')
                        if rtt_target is not None and pow_hash > rtt_target:
                            print 'Warning: block misses the eCash real-time target from the last template (%064x > %064x).' % (pow_hash, rtt_target)
                            print 'The RTT loosens over time, so the node may still accept it; otherwise it will be parked.'
