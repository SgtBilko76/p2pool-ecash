...
class Node(object):
    ...
    def get_current_txouts(self):
        return p2pool_data.get_expected_payouts(self.tracker, self.best_share_var.value, self.bitcoind_work.value['bits'].target, self.bitcoind_work.value.get('payout_subsidy', self.bitcoind_work.value['subsidy']), self.net)
