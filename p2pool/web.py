...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    def script_to_display_address(script):
        for ca_ver in [0, 1]: # cashaddr P2PKH, P2SH
            try:
                return bitcoin_data.script2_to_cashaddress(script, -1, ca_ver, node.net.PARENT)
            except bitcoin_data.AddrError:
                pass
        try:
            return bitcoin_data.script2_to_address(script, node.net.PARENT.ADDRESS_VERSION, -1, node.net.PARENT)
        except ValueError:
            return script.encode('hex')
    ...
    def get_local_stats():
        ...
        return dict(
            ...
            block_value_miners=node.bitcoind_work.value.get('payout_subsidy', node.bitcoind_work.value['subsidy'])*1e-8, # after reserved outputs
            reserved_outputs=[dict(address=script_to_display_address(o['script']), value=o['value']*1e-8)
                for o in node.bitcoind_work.value.get('reserved_outputs', [])], # e.g. eCash miner fund, staking reward
